			...
			.then(({ data }) => {
				axios
					...
					.then((resp) => {
						...
						data.forEach((c: CustomerEntry) => {
							...
							const filtered = orders.filter(
								(o: any) => o.customerId && o.customerId._id === c._id
							);
							filtered.forEach((o: any) => {
								...
								if (c.paid !== undefined) {
									...
									c.paid = parseFloat(c.paid.toFixed(2));
								}
					...
					.catch((err) => {
						console.error(err);
						message.error(
							'Something went wrong while getting the customer orders.'
						);
					});
			})
			.catch((err) => {
				console.error(err);
				message.error(
					'Something went wrong while getting the list of customers.'
				);
			});
